...
#define PULL_RANDOM_PLAYER(X,Y)		X = dzn_allPlayers call BIS_fnc_selectRandom; if (Z) then {dzn_allPlayers = dzn_allPlayers - [X];};
...
dzn_ra_fnc_setRoleAttributes = {
	...
	_unit setVariable ["raRole", [dzn_roleMapping, _roleId] call dzn_fnc_getValuerByKey, true];
	_unit setVariable ["raPic", [dzn_rolePicMapping, _roleId] call dzn_fnc_getValuerByKey, true];
};
	...
	case ( (count dzn_allPlayers) % 10 < 5): {
		for "_i" from 0 to dzn_squadCount do {
			...
			if !(_i in [dzn_squadCount - 1, dzn_squadCount]) then {
				...
				for "_j" from 0 to floor ((9 + (count dzn_allPlayers) % 10)/2) do { ASSIGN_SQUADMEMBER };
			};
