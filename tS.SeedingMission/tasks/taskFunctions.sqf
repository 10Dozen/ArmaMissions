...
dzn_fnc_TaskManager_init = {
	...
	"TaskManager_NewTask" addPublicVariableEventHandler {		
		if (TaskManager_NewTask) then {
			...
			(_taskSettings select 2) call compile preProcessFileLineNumbers format ["tasks\%1\task.sqf", (_taskSettings select 1) call dzn_task_tasks];	
		};
	};	
};
...
dzn_fnc_requestTask = {
	...
	if !( dzn_fnc_TaskManager_isTaskActive ) then {
		_dialogResult =	[
			"HQ - Request Task",
			[
