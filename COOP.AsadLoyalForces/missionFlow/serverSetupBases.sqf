...

{
	...
	_spots = synchronizedObjects _x;
	{
		...
		#define	checkSpotType(MARK)	([MARK, str(_this), false] call BIS_fnc_inString)
		#define assignClassAndKit(MARK)	case checkSpotType(MARK): {_spotItem = call complie MARK; _kitAssigned = MARK;};
		...
		if !(isNil {_spotItem}) then {
			...
			if (typename _spotItem == "ARRAY") then {
